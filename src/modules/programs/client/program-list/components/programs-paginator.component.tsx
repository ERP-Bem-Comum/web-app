...
import { paginator, label, button, perPageWrap, perPageSelect } from './programs-paginator.css.ts'

const PER_PAGE_OPTIONS = [5, 10, 25] as const

export type ProgramsPaginatorProps = Readonly<{
  page: number
  totalPages: number
  perPage: number
...
export function ProgramsPaginator(props: ProgramsPaginatorProps): ReactNode {
  return (
    <nav className={paginator} aria-label={props.labels.page}>
      ...
      <button type="button" className={button} disabled={props.page >= props.totalPages} onClick={props.onNext}>
        {props.labels.next}
      </button>
    </nav>
  )
