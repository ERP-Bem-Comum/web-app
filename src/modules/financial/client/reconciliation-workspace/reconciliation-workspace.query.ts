...
export const accountStatementPeriodQueryOptions = (
  ...
  queryKey: ['financial', 'reconciliation', 'statement-period', accountId, range?.from, range?.to] as const,
  queryFn: () =>
    reconciliationRepository.getAccountStatementPeriod({
      accountId,
      from: range?.from ?? '',
      to: range?.to ?? '',
    }),
  enabled: accountId !== '' && range !== null,
  staleTime: 30_000,
})
