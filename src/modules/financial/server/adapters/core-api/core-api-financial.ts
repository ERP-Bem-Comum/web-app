...
export const createCoreApiFinancialClient = (baseUrl: string): FinancialClient => {
  ...
  return {
    ...
    create: async (input, token) => {
      const r = await resultFetch<unknown>(docs, { method: 'POST', body: { ...input, asDraft: false }, token })
      if (isErr(r)) return err(mapHttpError(r.error))
      return detailToModel(r.value)
    },
    ...
    approve: async (input, token) => {
      const r = await resultFetch<unknown>(`${docs}/${input.id}/approve`, { method: 'POST', body: { version: input.version }, token })
      if (isErr(r)) return err(mapHttpError(r.error))
      return detailToModel(r.value)
    },
    undoApproval: async (input, token) => {
      const r = await resultFetch<unknown>(`${docs}/${input.id}/undo-approval`, { method: 'POST', body: { version: input.version }, token })
      if (isErr(r)) return err(mapHttpError(r.error))
      return detailToModel(r.value)
    },
