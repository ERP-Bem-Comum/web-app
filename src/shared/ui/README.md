...
- `atoms/` · `molecules/` — **componentes** ✅ implementados (spec 005): átomos **Button · Input · Checkbox · Logo · Card** + moléculas **Field · Badge · InputWithIcon**. Porta única `import { Button, Input, Checkbox, Logo, Card, Field, Badge } from '#shared/ui'`. Como usar: [`quickstart`](../../../specs/005-design-system-atoms/quickstart.md). A hierarquia (atoms ↛ molecules ↛ organisms) e a regra "só tokens" são enforçadas por lint — ver [`handbook/reference/design-system/lint-enforcement.md`](../../../handbook/reference/design-system/lint-enforcement.md). Anatomia de cada componente: ver abaixo.
- `organisms/` — **blocos compostos** ✅ fundação implementada (spec 009): **DataTable** (tabela genérica `<T>`: colunas configuráveis + estados loading/error/ready+empty) · **PageHeader** (título + subtítulo + slot de ações). Views BURRAS e **agnósticas de domínio** — recebem dados, callbacks e textos (i18n) por props. Porta única `import { DataTable, PageHeader, type Column } from '#shared/ui'`. Como usar: [`quickstart`](../../../specs/009-design-system-organisms/quickstart.md). _(Barra de controles, diálogo modal e layout de formulário virão em specs incrementais.)_
