...
- [X] T011 [P] Helper de RBAC (FR-020) em `src/modules/partners/client/data/helpers/can.ts` + teste ✅ (lógica pura pronta; **fonte resolvida (Rev. 2)**: core-api `/me` já entrega `permissions[]` — falta só a ponte no `auth` front: estender `MeSchema`/`AuthUser`/`CurrentUser` p/ `can()` ler `me.permissions`. Auth é protegido — liberar edição ou fazer do lado de vocês)
...
> 🚧 **Progresso**: domínio NUCLEAR iniciado e verde (5 testes) — `Collaborator` (status duplo
> registration×activation) + transições puras `buildPreRegistration`/`completeRegistration` (unidirecional)/
> `deactivate`/`reactivate` (`collaborator.types.ts`, `collaborator.ts`). **Falta**: os 21 campos do
> detalhe (complete-registration), parser CSV (T021/T027), use-cases (T028), adapters/server-fns (T029/T030),
> client (T032–T035) e UI (T036). T020/T026 parcialmente cobertos.
