...
- **CSS files**: When [`build.cssCodeSplit`](/config/build-options#build-csscodesplit) is `false`, a single CSS file is generated with the key `style.css`. When `build.cssCodeSplit` is not `false`, the key is generated similar to JS chunks (i.e. entry chunks will not have `_` prefix and non-entry chunks will have `_` prefix).
...
## Step 4: Rendering Links in Your Backend
