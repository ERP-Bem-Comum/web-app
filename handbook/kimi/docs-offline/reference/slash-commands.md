<!-- Offline capture de https://moonshotai.github.io/kimi-code/en/reference/slash-commands.html (2026-06-02). Versão Node/TypeScript. -->
...
| `/provider` | — | Open the interactive provider manager to view, add, and delete configured providers. | Yes |
...
| `/export-debug-zip` | — | Export the current session as a debug ZIP archive. The archive always includes the active global diagnostic log. | No |
...
| `/plugins` | — | Open the interactive plugin manager for user/global installs: install, inspect, enable, disable, confirm removal, reload, browse the official marketplace, and toggle plugin MCP servers. | Yes |
...
Skill types that can be exposed as slash commands include `prompt`, `inline`, `flow`, and skills without an explicitly declared type.
