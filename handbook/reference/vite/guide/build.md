...
[Watch an interactive lesson on Scrimba](https://scrimba.com/intro-to-vite-c03p6pbbdq/~037q?via=vite "Building for Production")
...
The build can be customized via various [build config options](/config/build-options). Specifically, you can directly adjust the underlying [Rolldown options](https://rolldown.rs/reference/) via `build.rolldownOptions`:
...
```js
// vite.config.js (multiple entries)
import { dirname, resolve } from 'node:path'
import { defineConfig } from 'vite'
