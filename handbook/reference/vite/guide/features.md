...
To specify the target in dev, the [`oxc.target`](/config/shared-options#oxc) option can be used, which defaults to `esnext` for minimal transpilation. In builds, the [`build.target`](/config/build-options#build-target) option takes higher priority over `oxc.target` and can also be set if needed.
...
There is experimental support for using Lightning CSS for CSS processing entirely. You can opt into it by adding [`css.transformer: 'lightningcss'`](./../config/shared-options#css-transformer).
...
You can also use the `query` option to provide queries to imports, for example, to import assets [as a string](/guide/assets#importing-asset-as-string) or [as a url](/guide/assets#importing-asset-as-url):
...
Vite can generate a file of all the dependencies' licenses used in the build with the [`build.license`](/config/build-options#build-license) option. It can be hosted to display and acknowledge the dependencies used by the app.
...
To serve the file at a different path, you can pass `{ fileName: 'license.md' }` for example, so that it's served at `https://example.com/license.md`. See the [`build.license`](/config/build-options#build-license) docs for more information.
