...
With BuildKit enabled, building the `stage2` target in this Dockerfile means only `base` and `stage2` are processed. There is no dependency on `stage1`, so it's skipped. The legacy builder processes `stage1`, even if `stage2` doesn't depend on it.
