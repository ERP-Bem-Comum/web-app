...
description: Especialista em TanStack Start (front + BFF) — server functions (createServerFn), SSR, middleware, env vars, auth primitives e cookies de sessão.
type: prompt
whenToUse: Ao criar endpoints BFF, fluxo de auth, ou lidar com fronteira servidor/cliente.
---
...
Contexto: o BFF fala com o `core-api` (interno). Para dúvidas do backend, use `/skill:core-api-consultant` (se disponível) ou consulte `handbook/core-api/`. Cite o doc ao responder.
